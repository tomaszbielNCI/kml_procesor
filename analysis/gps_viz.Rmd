...
```{r plot27}
# Trellis (facet) plot – altitude profiles for top routes (Cedric Scherer style)

# Ensure top_routes is defined (fallback if not already computed)
...
# Prepare data – filter, compute time from start, sample up to 200 points per route
trellis_data <- gps_data %>%
  filter(!is.na(altitude), !is.na(time), track_name %in% top_routes) %>%
  arrange(track_name, time) %>%
  group_by(track_name) %>%
  mutate(
  ...
  slice_sample(n = pmin(200, total_points)) %>%  # take up to 200 points per route (all if fewer)
  ungroup()
...
if (nrow(trellis_data) > 0) {
  ggplot(trellis_data, aes(x = time_minutes, y = altitude)) +
    ...
    labs(
      ...
      subtitle = "Each panel shows a different route (top 10 by number of points)",
      x = "Time from start (minutes)", y = "Altitude (m)"
    ) +
  ...
  message("No data available for trellis plot – check if top_routes contains valid track names.")
}
```
...
# Select specific route: 'slieve lieage 28/09/2025 13:16'
selected_route <- "slieve lieage 28/09/2025 13:16"
