...
gps_data %>% 
  ...
  coord_flip() +
  labs(title = "Altitude Distribution for Top 10 Most Frequent Routes",
...
## 6. Altitude vs Longitude

```{r plot6}
ggplot(gps_data, aes(x = longitude, y = altitude)) +
  geom_point(alpha = 0.3, color = "darkgreen") +
  geom_smooth(method = "loess", color = "red", se = TRUE) +
  labs(title = "Altitude vs Longitude Relationship",
       x = "Longitude", y = "Altitude (m above sea level)")
```
...
```{r plot19}
gps_data %>%
  filter(!is.na(route_total_calories) & !is.na(route_distance_km)) %>%
  ggplot(aes(x = route_distance_km, y = route_total_calories)) +
  geom_point(alpha = 0.6, color = "darkred") +
  geom_smooth(method = "lm", color = "blue", se = TRUE) +
  labs(title = "Calories vs Distance Relationship",
       x = "Distance (km)", y = "Calories") +
  annotate("text", x = Inf, y = -Inf, 
           label = paste("Correlation:", round(cor(route_distance_km, route_total_calories, use = "complete.obs"), 2)),
           hjust = 1.1, vjust = -0.5)
```
